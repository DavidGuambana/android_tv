...
public class VideoDetailsFragment extends DetailsSupportFragment {
    ...
    private void setupRelatedMovieListRow() {
        ...
        // Crea un adaptador para la fila de películas relacionadas
        ...
        for (int categoryIndex = 0; categoryIndex < MovieList.MOVIE_CATEGORY.length; categoryIndex++) {
            ...
            // Agrega hasta NUM_COLS películas de la categoría actual a la fila
            for (int j = 0; j < Math.min(categoryMovies.size(), NUM_COLS); j++) {
                listRowAdapter.add(categoryMovies.get(j));
            }
        }

        // Crea un encabezado para la fila de películas relacionadas
        HeaderItem header = new HeaderItem(0, subcategories[0]);

        // Agrega la fila configurada al adaptador principal
        mAdapter.add(new ListRow(header, listRowAdapter));

        // Configura un presentador para la clase ListRow
